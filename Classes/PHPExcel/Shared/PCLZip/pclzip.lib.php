    ...
    if ($v_size > 0) {
      ...
      if ((is_integer($v_arg_list[0])) && ($v_arg_list[0] > 77000)) {
        ...
        if (isset($v_options[PCLZIP_OPT_ADD_PATH])) {
          ...
          if ((strlen((string) $v_path) > 0) && (!str_ends_with($v_path, '/'))) {
            $v_path .= '/';
          }
...
//      eval('$v_result = '.$p_options[PCLZIP_CB_POST_EXTRACT].'(PCLZIP_CB_POST_EXTRACT, $v_local_header);');
  ...
  function PclZipUtilPathInclusion($p_dir, $p_path)
  {
    ...
    if (   ($p_dir == '.')
        || ((strlen((string) $p_dir) >=2) && (str_starts_with($p_dir, './')))) {
      $p_dir = PclZipUtilTranslateWinPath(getcwd(), FALSE).'/'.substr((string) $p_dir, 1);
    }
    if (   ($p_path == '.')
        || ((strlen((string) $p_path) >=2) && (str_starts_with($p_path, './')))) {
  ...
  function PclZipUtilTranslateWinPath($p_path, $p_remove_disk_letter=true)
  {
    if (stristr(php_uname(), 'windows')) {
      ...
      if ((strpos((string) $p_path, '\\') > 0) || (str_starts_with($p_path, '\\'))) {
          $p_path = strtr($p_path, '\\', '/');
      }
    }
    return $p_path;
